...
class Node:
	...
	def process_node( self, all_words, my_words_idx, history, verbose ):
		...
		for idx in my_words_idx:
			bg_list = Node.get_bigrams( all_words[ idx ] )
...
#         if len(split_dict) < 2 and verbose:
#             print("Warning: did not make any meaningful split with this query!")
...
# # # Example usage
# # dictionary = ['area', 'rear', 'optional', 'proportional', 'beside', 'besides']
# # dt = my_fit(dictionary, verbose=True)
# # bigrams = ('al', 'io', 'na', 'on', 'op')
# # predictions = my_predict(dt, bigrams)
# # print(predictions)

# # Example usage
# if __name__ == "__main__":
#     # Load the dictionary from the uploaded file
#     with open("C:/Users/Mritunjay Maddhesiya/OneDrive/Desktop/machine/assn2/dummy/dict_secret", "r") as f:
#         dictionary = [line.strip() for line in f.readlines()]
    
#     model = my_fit(dictionary, verbose=True)
    
#     # Example test bigrams
#     test_bigrams = ('ar', 'ea', 're')
#     predictions = my_predict(model, test_bigrams)
